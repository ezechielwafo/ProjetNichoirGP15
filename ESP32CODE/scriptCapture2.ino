...
const int LED_PIN = 33;               // LED rouge intégrée (ou GPIO 4 pour flash)
    ...
    <div class='container'>
        ...
        <form action='/login' method='POST'>
            <div class='form-group'>
                ...
                <input type='text' id='ssid' name='ssid' value='Orange-f6a09' required>
            </div>
            <div class='form-group'>
...
void handlePIRDetection() {
    ...
    // Détection d'un nouveau mouvement (transition LOW → HIGH)
    if (currentPirState == HIGH && lastPirState == LOW) {
        // Anti-rebond: vérifier le délai minimum
    ...
    // Fin de détection (transition HIGH → LOW)
...
void loop() {
    ...
    if (systemReady && wifiAuthenticated && WiFi.status() == WL_CONNECTED) {
        ...
        // GESTION PIR - PRIORITÉ ABSOLUE
        if(pirCalibrated) {
            handlePIRDetection();
        }

        // Photos automatiques toutes les minutes (en plus du PIR)
        unsigned long currentTime = millis();
    ...
    delay(50);  // Réduction à 50ms pour réactivité PIR
}